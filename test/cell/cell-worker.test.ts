...
import type { CellEngine } from "../../src/cell/cell-engine";
import {
 ...
 */
function makeHarness(engine: CellEngine, ambient?: AmbientHandler) {
					...
					.catch((err) =>
						push(
							`${JSON.stringify({
								...
								...((err as { __infra__?: boolean }).__infra__ ? { infrastructure: true } : {}),
							})}\n`,
						),
					);
...
for (const [engineName, makeEngine] of ENGINES) {
	describe(`cell worker (${engineName} engine)`, () => {
		it("a lexically rejected cell fails loudly through the worker", async () => {
		...
		describe("realm containment", () => {
			...
			it("legitimate JS stdlib still works inside the realm", async () => {
				const h = makeHarness(makeEngine());
				const result = await h.runCell(
					[
			...
			it("ambient return values cannot be walked to a host constructor", async () => {
				const h = makeHarness(makeEngine(), async (method) => {
		...
		describe("ambient API", () => {
			it("routes ambient calls to the parent and returns the response", async () => {
				const h = makeHarness(makeEngine(), async (method, args) => {
			...
			it("ambient errors reject inside the cell and fail it when uncaught", async () => {
				const h = makeHarness(makeEngine(), async () => {
		...
		describe("infrastructure-error identity", () => {
			it("an uncaught infra-tagged rejection fails the cell as infrastructure", async () => {
				const h = makeHarness(makeEngine(), async () => {
					throw infraError("StoreUnavailable: store worker died");
				});
				const result = await h.runCell("return await get('x');");
				expect(result.ok).toBe(false);
				expect(result.error).toContain("StoreUnavailable");
				expect(result.infrastructure).toBe(true);
				await h.close();
			});

			it("a cell that catches infra and throws its OWN error is a stumble, not infra", async () => {
				const h = makeHarness(makeEngine(), async () => {
					throw infraError("StoreUnavailable: store worker died");
				});
				const result = await h.runCell(
					[
						"try { await get('x'); } catch (e) {",
						"  throw new Error('StoreUnavailable: store worker died');", // same TEXT, cell-forged
						"}",
					].join("\n"),
				);
				expect(result.ok).toBe(false);
			...
			it("rethrowing the SAME infra error object keeps the infrastructure tag", async () => {
				const h = makeHarness(makeEngine(), async () => {
					throw infraError("StoreUnavailable: store worker died");
				});
				const result = await h.runCell("try { await get('x'); } catch (e) { throw e; }");
				expect(result.ok).toBe(false);
				expect(result.infrastructure).toBe(true);
				await h.close();
			});
		...
		describe("engine teardown", () => {
			it("an un-awaited ambient call resolving AFTER the cell ends does not wedge the worker", async () => {
				const h = makeHarness(makeEngine(), async () => {
					await new Promise((r) => setTimeout(r, 25));
					return "late";
				});
				const first = await h.runCell("get('x'); return 'early';");
				expect(first.ok).toBe(true);
				expect(first.returnValue).toBe("early");
				// Let the late response land against the finished cell, then prove
				// the worker still serves.
				await new Promise((r) => setTimeout(r, 40));
				const second = await h.runCell("return 1 + 1;");
				expect(second.ok).toBe(true);
				expect(second.returnValue).toBe("2");
				await h.close();
			});
		});
		...
		describe("programs namespace", () => {
			...
			it("only exposes programs present in the request", async () => {
				const h = makeHarness(makeEngine(), async () => "ok");
...
describe("quickjs-only semantics", () => {
	it("a cell awaiting a promise nothing can resolve gets a typed deadlock error, not a hang", async () => {
		const h = makeHarness(new QuickJSCellEngine());
		const result = await h.runCell("await new Promise(() => {}); return 'unreachable';");
		expect(result.ok).toBe(false);
	...
	it("an un-cleared interval is torn down with the cell; the worker keeps serving", async () => {
		const h = makeHarness(new QuickJSCellEngine());
		const first = await h.runCell("setInterval(() => {}, 5); return 'left running';");
		expect(first.ok).toBe(true);
