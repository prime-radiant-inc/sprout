...
describe("SessionController", () => {
	...
	test("compact command while idle runs compaction immediately", async () => {
		...
		// Should have emitted a warning showing the compaction summary
		const compactionWarnings = events.filter(
			(e: any) => e.kind === "warning" && e.data.message?.includes("Compact"),
		);
		expect(compactionWarnings).toHaveLength(1);
		expect(compactionWarnings[0].data.message).toContain("compacted summary");
	});
...
describe("SessionController session-wide event wiring", () => {
	...
	test("starts root observer from static agent config", async () => {
		const bus = new EventBus();
	...
	test("does not attach a hard-coded observer without static config", async () => {
		...
		const fakeGenome = {
			getAgent: (name: string) =>
				name === "root" ? { name: "root", model: "best", observers: [] } : undefined,
		} as any;
		...
		new SessionController({
			...
			spawner: fakeSpawner,
			genome: fakeGenome,
			getResolverSettings: () => createResolverSettings([]),
		});
		let contextUpdates = 0;
		bus.onEvent((event) => {
			if (event.kind === "context_update") contextUpdates++;
	...
	test("retries failed delegate observer deliveries with original child context", async () => {
		const bus = new EventBus();
		...
		const fakeSpawner = {
			...
			updateSessionId: async () => {},
			clearHandles: async () => {},
			spawnAgent: async (options: { goal: string }) => {
				spawnGoals.push(options.goal);
				spawnAttempts++;
				if (spawnAttempts === 1) {
		...
		await waitFor(() => spawnGoals.length === 1);
		...
		await waitFor(() => spawnGoals.length === 2);
		await waitFor(() => messages.length === 1);
		expect(spawnGoals[1]).toContain("child one final text");
		expect(spawnGoals[1]).toContain("child one result");
		expect(messages[0]).toContain("child two final text");
		expect(messages[0]).toContain("child two result");
	});

	test("does not let repeatedly failed delegate observer deliveries starve newer frames", async () => {
		const bus = new EventBus();
		const spawnGoals: string[] = [];
		const messages: string[] = [];
		const fakeSpawner = {
			...
			spawnAgent: async (options: { goal: string }) => {
				spawnGoals.push(options.goal);
				if (options.goal.includes("child one final text")) {
		...
		await waitFor(() => spawnGoals.length === 3);
		await waitFor(() => messages.length === 1);
		expect(spawnGoals[2]).toContain("child two final text");
		expect(spawnGoals[2]).toContain("child two result");
		expect(messages[0]).toContain("child one final text");
		expect(messages[0]).toContain("child one result");
	});
