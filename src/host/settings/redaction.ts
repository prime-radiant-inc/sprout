export function redactCredentialText(value: string): string {
	return value
		...
		.replace(
			/\b((?:oauth|pasteback|paste back)(?:\s+callback)?\s+(?:code|state))(\s*[:=]\s*)[^\s&]+/gi,
			"$1$2[redacted]",
		)
		.replace(
			/\b((?:oauth|pasteback|paste back)(?:\s+callback)?\s+(?:code|state))\s+[^\s]+/gi,
			"$1 [redacted]",
		)
		.replace(/\b(raw\s+(?:code|state))\s+[^\s]+/gi, "$1 [redacted]")
		...
		.replace(
			/(["'])(access_token|refresh_token|id_token)\1(\s*[:=]\s*)(["'])[^"']+\4/g,
